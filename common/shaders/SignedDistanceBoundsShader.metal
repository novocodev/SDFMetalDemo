#ifndef __SPATIALPHOTONICS_PORTED_PRIMITIVES
#define __SPATIALPHOTONICS_PORTED_PRIMITIVES

#include "spatial_photonics_types"

SPATIALPHOTONICS_NAMESPACE_BEGIN
...
#define TAU (2*PI)
#define PHI (sqrt((float)5)*0.5 + 0.5)
...
float fHexagonCircumcircle(vec3 p, vec2 h) {
    vec3 q = abs(p);
    return max(q.y - h.y, max(q.x*sqrt((float)3)*0.5 + q.z*0.5, q.z) - h.x);
...
float fHexagonIncircle(vec3 p, vec2 h) {
    return fHexagonCircumcircle(p, vec2(h.x*sqrt((float)3)*0.5, h.y));
}
...
float fOpUnionColumns(float a, float b, float r, float n) {
    if ((a < r) && (b < r)) {
        vec2 p = vec2(a, b);
        float columnradius = r*sqrt((float)2)/((n-1)*2+sqrt((float)2));
        pR45(p);
        p.x -= sqrt((float)2)/2*r;
        p.x += columnradius*sqrt((float)2);
        if (fmod(n,(float)2) == (float)1) {
            p.y += columnradius;
        }
...
float fOpDifferenceColumns(float a, float b, float r, float n) {
    ...
    if ((a < r) && (b < r)) {
        vec2 p = vec2(a, b);
        float columnradius = r*sqrt((float)2)/n/2.0;
        columnradius = r*sqrt((float)2)/((n-1)*2+sqrt((float)2));
        
        pR45(p);
        p.y += columnradius;
        p.x -= sqrt((float)2)/2*r;
        p.x += -columnradius*sqrt((float)2)/2;
...
enum nodeType {
    ...
    struct SDFScene {
        ...
        vec3 rayOrigin;
        uint nodeCount;
        float viewPortalWidth;
        float viewPortalHeight;
...
//Dynmaic distance functions for scene
//Ferformance is non optimal, minimum number of distance
//functions should be processed here
vec2 map( vec3 pos, constant SDFScene &scene );
vec2 map( vec3 pos, constant SDFScene &scene )
{
    uint nodeCount = scene.nodeCount;
    vec3 mpos = vec3(pos[0],pos[1],pos[2]);
    float cell = 0;
    
    Stack ds = newstack();
    
    for(uint i=0; i<nodeCount; i++) {
        SDFNode currNode = scene.nodes[i];
            
        switch (currNode.type) {
            case fPlaneType:
            {
               //distances[i] = vec2(fPlane(pos,vec3(currNode.floats[0],currNode.floats[1],currNode.floats[2]),0.0),currNode.renderScalar);
            }
            break;
            case fSphereType:
            {
                push(ds, spfSphere(mpos, currNode, i));
            }
                break;

            case fBoxCheapType:
            {
                push(ds, spfBoxCheap(mpos, currNode, i));
                //distances[i] = {fBoxCheap(pos - vec3{currNode.floats[0],currNode.floats[1],currNode.floats[2]},vec3{currNode.floats[3],currNode.floats[4],currNode.floats[5]}),currNode.renderScalar};
            }
                break;
                                /*
            case fRoundBoxType:
            {
                //distances[i] = {fRoundBox(pos - vec3{currNode.floats[0],currNode.floats[1],currNode.floats[2]},vec3{currNode.floats[3],currNode.floats[4],currNode.floats[5]},currNode.floats[6]),currNode.renderScalar};
            }
                break;
            case fTorusType:
            {
                //distances[i] = {fTorus(pos - vec3{currNode.floats[0],currNode.floats[1],currNode.floats[2]},currNode.floats[3],currNode.floats[4]),currNode.renderScalar};
            }
                break;
            case fTorus82Type:
            {
                //distances[i] = {fTorus82(pos - vec3{currNode.floats[0],currNode.floats[1],currNode.floats[2]},vec2{currNode.floats[3],currNode.floats[4]}),currNode.renderScalar};
            }
                break;
            case fTorus88Type:
            {
                //distances[i] = {fTorus88(pos - vec3{currNode.floats[0],currNode.floats[1],currNode.floats[2]},vec2{currNode.floats[3],currNode.floats[4]}),currNode.renderScalar};
            }
                break;
            case fCapsuleType:
            {
                //distances[i] = {fCapsule(pos,vec3{currNode.floats[0],currNode.floats[1],currNode.floats[2]},vec3{currNode.floats[3],currNode.floats[4],currNode.floats[5]},currNode.floats[6]),currNode.renderScalar};
            }
                break;
            case fTriPrismType:
            {
                //distances[i] = {fTriPrism(pos - vec3{currNode.floats[0],currNode.floats[1],currNode.floats[2]},vec2{currNode.floats[3],currNode.floats[4]}),currNode.renderScalar};
            }
                break;
            case fCylinderType:
            {
                //distances[i] = {fCylinder(pos - vec3{currNode.floats[0],currNode.floats[1],currNode.floats[2]},currNode.floats[3],currNode.floats[4]),currNode.renderScalar};
            }
                break;
            case fCylinder6Type:
            {
                //distances[i] = {fCylinder6(pos - vec3{currNode.floats[0],currNode.floats[1],currNode.floats[2]},vec2{currNode.floats[3],currNode.floats[4]}),currNode.renderScalar};
            }
                break;
            case fConeType:
            {
                //distances[i] = {fCone(pos - vec3{currNode.floats[0],currNode.floats[1],currNode.floats[2]},currNode.floats[3],currNode.floats[4]),currNode.renderScalar};
            }
                break;
                 */
            case pUnionType:
            {
                push(ds, sppU( pop(ds), pop(ds)));
            }
                break;
            case pSubtractionType:
            {
                push(ds, sppS( pop(ds), pop(ds), currNode, i));
                //distances[i] = pU(distances[currNode.ints[0]],distances[currNode.ints[1]]);
            }
                break;
            case pModOffsetType:
            {
                sppModOffset(mpos, currNode);
            }
                break;
            case pModPolarType:
            {
                //reset cell and mpos to raw pos value, do we need to do this?
                //mpos = vec3(0);
                //cell = 0;

                vec2 tmpPos = vec2(mpos.xz);

                cell = sppModPolar(tmpPos,currNode);
                
                mpos = vec3(tmpPos[0],mpos[1],tmpPos[1]);
...
vec2 castRay( vec3 ro, vec3 rd, constant SDFScene &scene);
vec2 castRay( vec3 ro, vec3 rd, constant SDFScene &scene)
{
    float tmin = 1.0;
    float tmax = 250.0;
    ...
    for(; i<50; i++ )
    {
        vec2 res = map( ro+rd*t, scene);
        if( res.x<precis || t>tmax ) break;
        t += res.x;
        m = res.y;
    }
    
    if( t>tmax || i == 50) m=-1.0;
    return vec2( t, m );
}

float softshadow( vec3 ro, vec3 rd, float mint, float tmax, constant SDFScene &scene );
float softshadow( vec3 ro, vec3 rd, float mint, float tmax, constant SDFScene &scene )
{
    float res = 1.0;
    float t = mint;
...
vec3 calcNormal( vec3 pos, constant SDFScene &scene  );
vec3 calcNormal( vec3 pos, constant SDFScene &scene  )
{
    vec3 eps = vec3( 0.001, 0.0, 0.0 );
    vec3 nor = vec3(
        map(pos+eps.xyy, scene).x - map(pos-eps.xyy, scene).x,
        map(pos+eps.yxy, scene).x - map(pos-eps.yxy, scene).x,
        map(pos+eps.yyx, scene).x - map(pos-eps.yyx, scene).x );
 ...
 */
float calcAO( vec3 pos, vec3 nor , constant SDFScene &scene );
float calcAO( vec3 pos, vec3 nor , constant SDFScene &scene )
{
    float occ = 0.0;
    float sca = 1.0;
...
vec3 render( vec3 ro, vec3 rd, constant SDFScene &scene);
vec3 render( vec3 ro, vec3 rd, constant SDFScene &scene)
{
    
    //Set default background colour
    vec3 col = vec3(0.7, 0.6, 0.7) +rd.y*0.8;

    
    vec2 res = castRay(ro,rd, scene);
    ...
    int m = scene.nodes[int(res.y)].materialId;
        ...
        // lighitng
        float occ = calcAO( pos, nor , scene);
        ...
        dif *= softshadow( pos, lig, 0.02, 2.5 , scene);
        dom *= softshadow( pos, ref, 0.02, 2.5 , scene);
                              ...
                              uint2 gid [[thread_position_in_grid]])
{
    ...
    vec2 iResolution = vec2{scene.viewPortalWidth,scene.viewPortalHeight};
                                       ...
                                       uint2 gid [[thread_position_in_grid]])
    {
        ...
        vec2 res = castRay(scene.rayOrigin,rd, scene);
    ...
    SPATIALPHOTONICS_NAMESPACE_END
    
#endif /* __SPATIALPHOTONICS_DISTANCE_FUNCTIONS */
